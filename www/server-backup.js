...
app.post('/api/addresult', (req, res) => {
  async function addResults(){
       ...
       */
      const uri = "mongodb+srv://christer:Ytrewq-73-@oerdev.bqfcd.mongodb.net/assessment?retryWrites=true&w=majority";
...
app.get('/api/results/sessionid/:sessionId', (req, res) => {

async function findResultsSi(){
  ...
  const cursor = client.db("assessment").collection("results")
        .find({
            sessionId: { $eq: req.params.sessionId }
        })
            .sort({ userId: -1 });
...
findResultsSi().catch(console.error);
...
async function findResultsUi(){
  ...
  const uri = "mongodb+srv://christer:Ice278787@gdl.bqfcd.gcp.mongodb.net/assessment?retryWrites=true&w=majority";
  const client = new MongoClient(uri, { useNewUrlParser: true }, { useUnifiedTopology: true });
